...
export function ProductCard({ product }: Props) {
  ...
  const href =
    product.productPermalink ?? `/product/${product.sku.toLowerCase()}`;
    ...
    >
      ...
      <div className="sm:col-span-6 flex min-w-0 flex-col gap-2">
        ...
        <dl className="mt-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-1 text-xs">
          {product.condition && (
            <>
              <dt className="font-bold uppercase tracking-wide text-theme-muted text-[10px]">
                Condition
              </dt>
          ...
          {product.size && (
            <>
              ...
              <dd className="text-theme-dark-2">
      ...
      <div className="sm:col-span-3 flex flex-row sm:flex-col items-center sm:items-end justify-between gap-3">
        ...
        <div className="flex w-full max-w-45 flex-col gap-2">
          <button className="w-full rounded-md bg-theme-primary px-4 py-2.5 text-sm font-extrabold text-white text-center transition-colors hover:bg-theme-primary-dark">
            Quick View
          </button>
          <a
            href="tel:8889779085"
