...
export function MobileTrustSection() {
  ...
  return (
    <section className="lg:hidden" aria-label="Delivery help and guarantees">
      {/* Call banner. The whole block is the link, not just the number — on a
          phone the entire banner is the tap target it looks like. */}
      <Link
        href={CONTACT_TEL}
        className="relative mt-10 block bg-theme-primary px-4 pb-4 pt-8 text-center text-white transition-colors hover:bg-theme-primary-dark focus:outline-none focus-visible:ring-2 focus-visible:ring-theme-primary focus-visible:ring-offset-2"
      >
        ...
        <ul className="mt-6 grid grid-cols-2 gap-x-4 gap-y-5">
          <li className="flex gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-[11px] leading-snug text-theme-muted">
                ...
                <Link
                  href="#reviews"
                  className="underline hover:text-theme-dark"
                >
                  Read our reviews
                </Link>
              </p>
          ...
          {ASSURANCES.map(({ Icon, title, body }) => (
            <li key={title} className="flex gap-2.5">
              <Icon
                className="mt-0.5 h-4.5 w-4.5 shrink-0 text-theme-primary"
                aria-hidden
              />
              <div className="min-w-0">
