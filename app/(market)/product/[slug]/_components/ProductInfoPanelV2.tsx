...
/**
 * Product info panel, version 2 — the Selected Container Summary without the
 * Distance, Delivery and Sales tax rows.
 *
 * A copy of ProductInfoPanel rather than a prop on it, on purpose: the two are
 * meant to diverge, and the admin switch in `lib/productPanel.ts` chooses
 * between them per request. The cost is real — a change to the shared 90% has
 * to be made twice — so when one of these wins, delete the other rather than
 * leaving both to drift.
 *
 * Everything below this header is identical to ProductInfoPanel except the
...
export function ProductInfoPanelV2({
  product,
  relatedProducts,
  onVariantChange,
  locationChange,
    ...
    <div className="w-full">
      ...
      <div className="-mx-4 mt-6 border-t-2 border-theme-primary px-4 pt-3 sm:mx-0 sm:px-0">
        ...
        <ul className="mt-3 text-[14px] tabular-nums">
          ...
          {/* V2 stops here. The default panel follows Unit price with
              Distance, Delivery and Sales tax; this version leaves the
              summary describing the container alone and lets the ordering
              block below carry the delivery estimate. Removing the delivery
              rows removes the error row with them — it only ever explained
              why a delivery price was missing. */}
        </ul>
      </div>
