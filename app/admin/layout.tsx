...
//
// The production gate is three layers: proxy.ts 404s the path outright (that's
// the one users actually hit, and the one that returns a real 404 status), the
// Server Actions re-check for themselves because a form's POST target is its
// own endpoint, and this layout refuses to render the UI.
//
// This layer renders a dead page rather than calling notFound(). Under
// cacheComponents these routes are prerendered at build, so a throw here fires
// during prerender and dumps a NEXT_HTTP_ERROR_FALLBACK;404 stack for every
// admin route into every production build — noise that would hide real build
// errors. The status code is the proxy's job anyway; this layer's only job is
// to make sure no admin markup or data ever reaches a production response.
