...
> **Important context:** `app/(market)/checkout/_components/CheckoutClient.tsx` exists, but it's currently a **"Reserve My Container"** lead-capture flow, not a payment checkout — there's no card/payment UI at all, tax is a hardcoded `0.08875` constant computed client-side, and "Place Order" just flips local state (`setReserved(true)`) with a disclaimer that reserving isn't a purchase commitment. Every row below needs real payment UI built into this page before it can call anything, not just a wiring change.
>
> **Rollup #9 — blocked, picked back up 2026-07-14. Three things needed before building can resume:**
> 1. Real Braintree sandbox credentials (`BRAINTREE_MERCHANT_ID`/`PUBLIC_KEY`/`PRIVATE_KEY`) — `.env.local` has none.
> 2. reCAPTCHA keys — `/api/braintree_checkout` already calls `verifyRecaptcha()` needing `RECAPTCHA_SECRET_KEY`, plus a client-side site key (`NEXT_PUBLIC_RECAPTCHA_SITE_KEY`, doesn't exist yet, no widget built either). **Open question for the user**: is reCAPTCHA actually required on checkout, or should that check be dropped?
> 3. The real contract for `get-total`/`checkout` — `CheckoutPayload`/`OrderTotal` in `types/order.ts` are explicit placeholders. Specifically unconfirmed:
>    - **Order of operations**: client gets nonce → `POST /api/braintree_checkout` charges it → `POST /api/orders/checkout` is called with the resulting `transaction_id` (what the current `CheckoutPayload` type assumes)? Or does Django itself charge Braintree server-side inside `/api/orders/checkout`?
>    - `get-total`'s real tax/shipping/total field names.
>    - `checkout`'s real response shape (same as `Order` from `/api/auth/orders`? something else?).
>    A reference implementation doc (like `ORDER_HISTORY_ANSWER.md`/`REVIEWS_FLOW.md` for the prior two rollups) would settle this fastest; otherwise needs live sandbox testing once credentials exist — but not worth firing blind against a real payment gateway.
>
> Also an open decision (not a blocker, just needs picking): **Braintree Drop-in UI** (`braintree-web-drop-in`, not yet installed — recommended, less PCI surface) vs. custom card fields.

- [ ] 🟡 **Cart/Checkout page, qty/shipping/coupon change** → `POST /api/orders/get-total`
  Checkout computes subtotal/tax/total client-side with a flat tax rate. Not calling `get-total`.
- [ ] ❌ **Checkout page, on mount** (init Braintree Drop-in UI) → `GET /api/braintree_token`
  Not built — no Braintree Drop-in (or any payment field) is mounted in `CheckoutClient.tsx` yet.
- [ ] ❌ **Checkout page, "Place Order" button** → `POST /api/braintree_checkout`
  Not built — current button is "Reserve My Container Today," a local-only action.
- [ ] ❌ **Checkout page, after payment nonce received** → `POST /api/orders/checkout`
  Not built — no payment step exists yet to produce a nonce.
...
9. **Real payment checkout** — the biggest gap remaining. `CheckoutClient.tsx` needs an actual payment step added (Braintree Drop-in UI, nonce handling, real tax/total from `get-total`, order creation on success) — today it's a no-payment "reserve" flow end to end. This is also the missing piece for a real "post-checkout success → close cart" trigger.
