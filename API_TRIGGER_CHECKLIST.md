...
- [x] ✅ **Register page/form** — submit → `POST /api/register`
  `RegisterForm.tsx` exists and submits to `/api/auth/register` (mapped). The fake "I'm not a robot" checkbox is gone — replaced with the real `Recaptcha` widget (`components/ui/Recaptcha.tsx`, moved out of `checkout/_components/` since it's now shared by two routes), rendered only when `NEXT_PUBLIC_RECAPTCHA_SITE_KEY` is set. `/api/auth/register` now calls `verifyRecaptcha()` server-side, same opt-in pattern as `/api/braintree_checkout` (`isRecaptchaConfigured()` — skipped entirely while the env vars are blank). Verified live: register page renders with no console errors and no leftover placeholder text.
...
>
> **New blocker found 2026-07-15 — bigger than credentials:** tested `/api/orders/checkout` live against the real backend (bypassed payment, used the test account's real cart) and confirmed our `CheckoutPayload` contract is fine, but **the Django endpoint itself always 500s** — `ValueError: Currency formatting is not possible using the 'C' locale.` in `app/orders/views.py` line 89 (`locale.currency(...)` with no locale configured on the server). Worse: **the order is saved before the crash**, confirmed by real order records showing up despite every attempt 500ing — so once Braintree credentials exist, a real customer would have their card charged and order created, but see a broken checkout page. This needs a backend fix (configure the server locale, or stop using `locale.currency()`) — it's not something the frontend can work around. Also confirmed live: sending `payment_method: "braintree"` does not by itself move order `status` to `"paid"` — something else server-side controls that.
...
- [x] ✅ **Cart/Checkout page, qty/shipping/coupon change** → `POST /api/orders/get-total`
  Debounced (600ms) effect in `CheckoutClient.tsx` calls this whenever cart items or the shipping ZIP/country change, and replaces the flat client-side estimate with the real subtotal/tax/shipping/total once it resolves (falls back to the flat `0.08875` estimate until then, or if the call fails). **Verified live**: real response shape confirmed by hitting the backend directly with a real product (`{sub_total, total_tax, total_shipping, total_price, items_count, message}` — different field names than the original guessed `OrderTotal` type, now corrected). Confirmed tax is genuinely location-based, not flat — a 90210/US test returned 10.5%, not 8.875%. Confirmed live in the actual UI too: entering a ZIP updates the displayed Sales Tax from $0.00 to the real computed amount. The Braintree charge amount (`handlePlaceOrder`) already used the `total` variable, so it automatically now charges the real total, not the flat estimate.
...
- [x] ✅ **Checkout address form, ZIP field blur** → autofill city/state (`api.zippopotam.us`, external)
  New `lib/zippopotam.ts`'s `lookupZip(zip, country)` — called directly from the browser (no backend proxy, per the plan), maps the on-page country label (`'United States (US)'`/`'Canada (CA)'`) to the ISO2 path segment (`us`/`ca`). Wired into `AddressFields`' ZIP input `onBlur` in `CheckoutClient.tsx`, applies to both shipping and billing forms (shared component). **Verified live** against the real API: ZIP `90210` correctly autofilled City "Beverly Hills" / State "CA". Distinct from the app's *other* ZIP-based feature (Geoapify depot/delivery-zone lookup in `useGeoapify.ts`/`DeliveryZipCheck.tsx`) — that one finds the nearest depot, this one only autofills the address form.
...
9. **Real payment checkout** — built and wired (Braintree Drop-in UI, nonce handling, order creation on success, real backend-computed totals). Two blockers now, not one: real Braintree credentials in `.env.local`, **and** a real backend bug in `/api/orders/checkout` that 500s on every call (see above) — the second one needs a backend code fix and isn't fixable from this app.
