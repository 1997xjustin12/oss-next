...
9. **Real payment checkout** — built and wired (Braintree Drop-in UI, nonce handling, order creation on success, real backend-computed totals), leaving exactly one blocker: real Braintree credentials in `.env.local`. Once credentials land, needs a real sandbox transaction test before calling this done — see the contract-risk note above.
