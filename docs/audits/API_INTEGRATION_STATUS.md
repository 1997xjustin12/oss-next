...
| Endpoint (per plan) | Method | Status | Notes |
|---|---|---|---|
| `/api/subscribers/subscribe` | POST | ✅ **Verified live** (2026-07-24) | Calls `subscribeToNewsletter()` (`services/subscriber.service.ts`). Wired into `/my-account/newsletter` (`NewsletterPanel.tsx`, in the account nav) via `lib/newsletter.ts`'s `subscribeToNewsletter(email)`. **Client confirmed the subscribe/unsubscribe flow works end-to-end against the real backend.** |
| `/api/subscribers/unsubscribe` | POST | ✅ **Verified live** (2026-07-24) | Calls `unsubscribeFromNewsletter()`. Wired into the same `/my-account/newsletter` panel's Unsubscribe action. Confirmed working in the same client check. |
| Logged-in status via `is_subscribed` | GET | ✅ **Verified live** (via `/api/auth/profile`) | **Correction (2026-07-24):** a status read *does* exist for logged-in users — the profile's `is_subscribed` boolean. Our `normalizeUser` was dropping it; now mapped to `User.isSubscribed`, and `NewsletterPanel.tsx` reads it and writes it back through the auth context after a toggle. **Confirmed working by the client 2026-07-24** — the account page reads the flag correctly and the toggle behaves against the real backend. |
